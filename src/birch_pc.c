...
const u8 *GetPokedexRatingText(u16 count)
{
    ...
    if (count == 200)
    {
        if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(SPECIES_BONDED_AGGRON), FLAG_GET_CAUGHT)
         || GetSetPokedexFlag(SpeciesToNationalPokedexNum(SPECIES_BONDED_ALAKAZAM), FLAG_GET_CAUGHT)) // Bonded_Aggron or Bonded_Alakazam is not counted towards the dex completion. If either of these flags are enabled, it means the actual count is less than 200.
            return gBirchDexRatingText_LessThan200;
    ...
    {
        if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(SPECIES_BONDED_AGGRON), FLAG_GET_CAUGHT)
         && GetSetPokedexFlag(SpeciesToNationalPokedexNum(SPECIES_BONDED_ALAKAZAM), FLAG_GET_CAUGHT)) // If both of these flags are enabled, it means the actual count is less than 200.
            return gBirchDexRatingText_LessThan200;
        return gBirchDexRatingText_DexCompleted;
    }

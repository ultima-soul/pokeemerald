...
static void Summary_ShowMonSprite(void)
{
    ...
    sFactorySelectScreen->unk294[1].field0 = CreateMonPicSprite_HandleBonded_Alakazam(species, otId, personality, TRUE, 88, 32, 15, 0xFFFF);
    ...
    for (i = 0; i < 3; i++)
    {
        for (j = 0; j < SELECTABLE_MONS_COUNT; j++)
        {
            if (sFactorySelectScreen->mons[j].selectedId == i + 1)
            {
                ...
                sFactorySelectScreen->unk294[i].field0 = CreateMonPicSprite_HandleBonded_Alakazam(species, otId, personality, TRUE, (i * 72) + 16, 32, i + 13, 0xFFFF);
...
static void Swap_ShowSummaryMonSprite(void)
{
    ...
    sFactorySwapScreen->unk2C.field0 = CreateMonPicSprite_HandleBonded_Alakazam(species, personality, otId, TRUE, 88, 32, 15, 0xFFFF); // BUG: otId and personality should be switched.
...
static void Swap_ShowMonSprite(void)
{
    ...
    sFactorySwapScreen->unk2C.field0 = CreateMonPicSprite_HandleBonded_Alakazam(species, otId, personality, TRUE, 88, 32, 15, 0xFFFF);
    gSprites[sFactorySwapScreen->unk2C.field0].centerToCornerVecX = 0;
    gSprites[sFactorySwapScreen->unk2C.field0].centerToCornerVecY = 0;

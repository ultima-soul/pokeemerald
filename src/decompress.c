...
static void DuplicateBonded_AlakazamTiles(void *pointer, s32 species);
...
void DecompressPicFromTable(const struct CompressedSpriteSheet *src, void* buffer, s32 species)
{
    if (species > NUM_SPECIES)
        ...
        LZ77UnCompWram(src->data, buffer);
    DuplicateBonded_AlakazamTiles(buffer, species);
}
...
void LoadSpecialPokePic(const struct CompressedSpriteSheet *src, void *dest, s32 species, u32 personality, bool8 isFrontPic)
{
    ...
    DuplicateBonded_AlakazamTiles(dest, species);
...
void DecompressPicFromTable_DontHandleBonded_Alakazam(const struct CompressedSpriteSheet *src, void* buffer, s32 species)
{
    if (species > NUM_SPECIES)
        LZ77UnCompWram(gMonFrontPicTable[0].data, buffer);
    else
...
void HandleLoadSpecialPokePic_DontHandleBonded_Alakazam(const struct CompressedSpriteSheet *src, void *dest, s32 species, u32 personality)
{
    bool8 isFrontPic;
    ...
    LoadSpecialPokePic_DontHandleBonded_Alakazam(src, dest, species, personality, isFrontPic);
}
...
static void DuplicateBonded_AlakazamTiles(void *pointer, s32 species)
{
    if (species == SPECIES_BONDED_ALAKAZAM)

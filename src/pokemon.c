...
{
    return GetUnownLetterByPersonality(personality) + 905; // TODO
}
...
void CopyPlayerPartyMonToBattleData(u8 battlerId, u8 partyIndex)
{
    ...
    gBattleMons[battlerId].formId = GetMonData(&gPlayerParty[partyIndex], MON_DATA_FORM_ID, NULL);
    ...
    gBattleMons[battlerId].type1 = gBaseStats[GetFormSpeciesId(gBattleMons[battlerId].species, gBattleMons[battlerId].formId)].type1;
    gBattleMons[battlerId].type2 = gBaseStats[GetFormSpeciesId(gBattleMons[battlerId].species, gBattleMons[battlerId].formId)].type2;
    gBattleMons[battlerId].ability = GetAbilityBySpecies(gBattleMons[battlerId].species, gBattleMons[battlerId].abilityNum, gBattleMons[battlerId].formId);
    ...
    *targetFormId = 0;
    ...
    case 0:
        ...
        for (i = 0; i < EVOS_PER_MON; i++)
        {
            ...
            case EVO_LEVEL_DAY:
                RtcCalcLocalTime();
                if (gLocalTime.hours >= 12 && gLocalTime.hours < 24 && gEvolutionTable[formSpeciesId][i].param <= level)
                {
                    *targetFormId = GetFormIdFromFormSpeciesId(gEvolutionTable[formSpeciesId][i].targetSpecies);
                    targetSpecies = GetFormSpeciesId(gEvolutionTable[formSpeciesId][i].targetSpecies, 0); // Get base species
                }
            ...
            case EVO_LEVEL_NIGHT:
                RtcCalcLocalTime();
                if (gLocalTime.hours >= 0 && gLocalTime.hours < 12 && gEvolutionTable[formSpeciesId][i].param <= level)
                {
                    *targetFormId = GetFormIdFromFormSpeciesId(gEvolutionTable[formSpeciesId][i].targetSpecies);
                    targetSpecies = GetFormSpeciesId(gEvolutionTable[formSpeciesId][i].targetSpecies, 0); // Get base species
                }
            ...
            case EVO_ITEM_HOLD_NIGHT:
                RtcCalcLocalTime();
                if (gLocalTime.hours >= 0 && gLocalTime.hours < 12 && heldItem == gEvolutionTable[formSpeciesId][i].param)
                {
                    heldItem = 0;
            ...
            case EVO_ITEM_HOLD_DAY:
                RtcCalcLocalTime();
                if (gLocalTime.hours >= 12 && gLocalTime.hours < 24 && heldItem == gEvolutionTable[formSpeciesId][i].param)
                {
                    heldItem = 0;
                    SetMonData(mon, MON_DATA_HELD_ITEM, &heldItem);
                    *targetFormId = GetFormIdFromFormSpeciesId(gEvolutionTable[formSpeciesId][i].targetSpecies);
...
u32 CanMonLearnTMHM(struct Pokemon *mon, u8 tm)
{
    ...
    {
        u32 mask = 1 << tm;
        return gTMHMLearnsets[formSpeciesId][0] & mask;
    }
    else
    {
        u32 mask = 1 << (tm - 32);
        return gTMHMLearnsets[formSpeciesId][1] & mask;
    }
}
...
void SetWildMonHeldItem(void)
{
    if (!(gBattleTypeFlags & (BATTLE_TYPE_LEGENDARY | BATTLE_TYPE_TRAINER | BATTLE_TYPE_PYRAMID | BATTLE_TYPE_PIKE)))
    {
        ...
        u8 formId = GetMonData(&gEnemyParty[0], MON_DATA_FORM_ID, 0);
        u16 formSpeciesId = GetFormSpeciesId(species, formId);
        u16 var1 = 45;
        u16 var2 = 95;
        ...
        if (gMapHeader.mapLayoutId == LAYOUT_ALTERING_CAVE)
        {
            ...
            else
            {
                if (rnd < var1)
                    return;
                if (rnd < var2)
                    SetMonData(&gEnemyParty[0], MON_DATA_HELD_ITEM, &gBaseStats[formSpeciesId].item1);
                else
                    SetMonData(&gEnemyParty[0], MON_DATA_HELD_ITEM, &gBaseStats[formSpeciesId].item2);
            }
        }
        else
        {
